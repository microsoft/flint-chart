...
const PARAM_DESCRIPTIONS: Record<string, string> = {
    ...
    showValueLabels: 'Print the numbers on the marks. Seeded from the theme’s own habit at this density; withheld when the marks are too dense to read. On a stacked bar each segment prints its own value, centred in the segment — or its share, where the stack is normalized.',
...
const ZH_PARAM_DESCRIPTIONS: Record<string, string> = {
    ...
    showValueLabels: '在标记上打印数值。默认值来自主题在当前密度下的习惯；标记过密时不提供该选项。堆叠柱状图会在每个分段中部打印该分段自身的数值；归一化堆叠时打印占比。',
