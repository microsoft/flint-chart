...
import FLINT_INDEX from './theme-lab-assets/_flint-index.json';
 ...
 * bespoke redesign of the same chart, plus the diff between them.
 *
 * Nothing on this page is generated by a theming engine. Every "themed" spec
 * in `theme-lab-assets/<id>.<theme>.json` was written by hand against the
 * corresponding `<id>.flint.json` baseline, so the third column is an honest
 * inventory of what a design-theme language would actually have to control.
 */
...
// Eagerly pull in every spec in the assets folder. `<id>.flint.json` is the
// baseline; `<id>.<theme>.json` is the hand-authored redesign.
const SPEC_MODULES = import.meta.glob('./theme-lab-assets/*.json', { eager: true }) as Record<
    string,
    { default: any }
>;
...
function buildRows(): LabRow[] {
    ...
    const flintById = new Map<string, any>();
    const themedById = new Map<string, any[]>();
    for (const [path, mod] of Object.entries(SPEC_MODULES)) {
        const file = path.split('/').pop()!;
        if (file.startsWith('_')) continue;
        const match = /^(.*)\.([a-z-]+)\.json$/.exec(file);
        if (!match) continue;
        const [, id, kind] = match;
        if (kind === 'flint') flintById.set(id, mod.default);
        else themedById.set(id, [...(themedById.get(id) ?? []), mod.default]);
    }

    const meta = new Map((FLINT_INDEX as FlintIndexEntry[]).map((e) => [e.id, e]));
    const rows: LabRow[] = [];
    for (const [id, themedList] of themedById) {
        const flint = flintById.get(id);
        if (!flint) continue;
        const info = meta.get(id);
        for (const themed of themedList) {
            rows.push({
                id,
                ...
                source: info?.source ?? '',
                rows: info?.rows ?? 0,
                theme: themed.__theme__ as ThemeId,
                flintSpec: flint,
                themedSpec: themed,
                design: (themed.__design__ as string[]) ?? [],
            });
        }
    }
...
/**
 * One wall tile = one (chart, language) pair, baseline left, redesign right,
 * small enough to scan a whole language in one screen. The argument for the
 * pair lives in the popup, not here.
 */
...
const THEMESPEC_BY_THEME = THEME_SPECS.themes as unknown as Record<string, unknown>;
...
function CoverageNotes({ rows }: { rows: LabRow[] }) {
    const index = FLINT_INDEX as FlintIndexEntry[];
